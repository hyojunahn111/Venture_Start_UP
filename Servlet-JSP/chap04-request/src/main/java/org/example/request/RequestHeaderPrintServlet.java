    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        * */
        ...
        Enumeration<String> headerNames = request.getHeaderNames();
            ...
            /*
            *   host                        : 서버의 도메인 네임과 서버가 현재 Listening 중인 TCP 포트로 지정(서버가 바라보고 있는 포트를 지정)
                connection                  : HTTP 통신이 완료된 후에 네트워크 접속을 유지할지 결정
                sec-ch-ua                   : 브라우저와 관련된 각 브랜드의 이르 및 주요 버전
                sec-ch-ua-mobile            : 모바일로 키면 여기에 찍힘
                sec-ch-ua-platform          : 윈도우로 키면 윈도우가 찍힘
                upgrade-insecure-requests
                user-agent                  : 현재 사용자가 어떤 클라이언트(OS, browser 포함)을 이용해 보낸 요청인지 명시
                accept                      : 요청을 보낼 때 서버에서 요청할 응답 타입 명시
