...
//        model.addAttribute("flashMessage1", "redirect model message");
        ...
        * 단, 세션이 임시로 값을 담았다가 소멸하는 방식이므로 세션에 동일한 키 값이 존재하지 않아야 한다*/
        rttr.addFlashAttribute("flashMessage1", "redirect flash message (string return)");
...
//        mv.addObject("flashMessage2", "redirect model message");
        rttr.addFlashAttribute("flashMessage2", "redirect flash message (string return)");
        mv.setViewName("redirect:/");
