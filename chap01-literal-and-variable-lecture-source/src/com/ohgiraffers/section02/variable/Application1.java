...
public class Application1 {
    public static void main(String[] args) {
        ...
        System.out.println("보너스를 포함한 금여 : " + (1000000 + 200000) + "원");
        /*정수가 담길 수 있는 공간인 int형으로 해놓으면 백만이 들어갔지만, 샐러리 = 급여라는 것을 알 수 있고, 20만원은 보너스 값이구나라는 것을 이해할 수 있다*/
...
//        System.out.println("보너스를 포함한 급여 : " + (salary + bonus) + "원");
        ...
        for(int i = 1; i<=10; i++){
            point += 10;
            System.out.println(i + "번 고객에게 포인트를" + point + " 지급하였습니다.");

        }

        /*3. 시간에 따라 변경되는 값을 저장하고 사용할 수 있다*/
        System.out.println("===== 변수에 저장된 값 변경 테스트=====");
        /*변수는 하나의 값을 저장하고 사용하기 위한 공간이기 보다, 변하는 값을 젖ㅇ하기 위한 공간이다*/
        int sum = 0;

        sum += 10;
        System.out.println("sum에 10을 더하면 현재 sum의 값은? " + sum);
        sum += 10;
        System.out.println("sum에 10이 있었으니 10을 추가로 더한 현재 sum의 값은? " + sum);
        sum += 10;
        System.out.println("sum에 20이 있었는데 추가로 10을 더 더하면? " + sum);

        /*동일한 sum이라는 변수를 세변 출력했지만, 출력하는 시점마다 수령된 연산의 결과에 따라 출력되는 sum의 값은 달라진다.*/
