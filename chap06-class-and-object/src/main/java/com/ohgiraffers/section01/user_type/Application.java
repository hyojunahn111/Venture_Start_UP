...
public class Application {
    public static void main(String[] args) {
        ...
        /*클래스가 무엇인지 이해하고 작성할수 있다*/
        /*지금까지 자바에서 제공되는 자료들을 취급하는 자료형에 대해 학습했다
        * 이제 조금 더 복잡한 자료를 취급할 수 있는 방법을 생각해보자
        * 회원 정보를 관리하기 위해 회원의 여러 정보(아이디, 비밀번호, 나이 ,이름, 성별 ,취미)를 취급하여 관리하려한다
        * 지금까지 배운 내용을 가지고 한 번 값을 저장하고 출력해보자*/
        ...
        /*이렇게 각각의 변수로 관리하게 되면 여러 가지 단점ㅇ 있다.
        ...
        * 이러한 공통점을 고려하여 그룹화 할 수있는 명사 형태로 ㄱ클래스를 작성할 것이다
        * Member라는 클래스를 추가해보자 */

        /*2. 사용자 정의의 자료형 사용하기*/
        /*2-1. 변수 선언 및 객체 생성*/
        /*자료형 변수명 = new 클래스명(); <- 지금까지 사용한 이 구문은 객체(instatnce)를 생성하는 구문이다
        * 사용자 정의 자료형인 클래스를 이용하기 위해서는 new 연산자로 heap에 할당을 해야한다
        * 객체를 생성하게 되면 클래스에 정의한 필드와 메소드 대로 객체(instance)가 생성된다
        * 아이디, 비밀번호, 이름, 나이, 성별, 취믈ㄹ 연속된 메모리 주소에서 사용하도록 heap에 공간을 만들었다(클래스에 작성한 내용대로 생성함)
        * */
        ...
        * 각 공간은 필드명으로 접근한다(배열은 인덱스로 접근, 객체는 필드명으로 접근이다.)*/
        ...
        System.out.println("변경 후 memeber.id : " + member.id);
        System.out.println("변경 후 memeber.pwd : " + member.pwd);
        System.out.println("변경 후 memeber.name : " + member.name);
        System.out.println("변경 후 memeber.age : " + member.age);
        System.out.println("변경 후 memeber.gender : " + member.gender);
        System.out.println("변경 후 memeber.hobby : " + member.hobby);
