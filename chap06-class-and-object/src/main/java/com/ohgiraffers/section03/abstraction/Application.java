...
public class Application {
    public static void main(String[] args) {
        ...
        * 현실 세계와 닮아있지만 현실 세계와 다른 점은 객체간의 상호 작용을 한다는 사실은
        * 현실 세계와 동일하지만 의인화 기법이 적용되어
        * 현실 세계에서 불가능한 무생물이나 개념 같은 존재들도 하나의 주체로
        * 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 현실 세계와는 다른 점이다(캡슐화의 의미)
        * 모든 객체는 책임을 가지고 자신의 역할을 수행해야한다
        ...
        * */
        ...
        /*추상화란?
        * 공통된 부분은 추출하고 공통되지 않은 부분을 제거한다는 의미를 가지며, 추상화의 목적은 유연성을 확보하기 위함이다
        * 유연성 확보는 여러 곳에서 적용될 수 있는 유연한 객체를 의미하며, 즉 재사용성이 높아질 수 있게 한다는 의미이다
        * 객체의 재사용성이 증가하면 중복 작성되는 코드를 줄일 수 있으며, 오류 발생 가능성을 감소시키고 유지보수성을 증가시킨다
        * */

        /*객체와 객체의 상호작용이란?
        * 객체와 객체는 메세지를 통해 서로 상호작용을 한다. 메시지(메소드 호출)을 보내는 쪽을 송신자라고 하고
        * 메세지를 수신하는 쪽을 수신잘 하고 한다. 수신자는 메세지를 전달 받아 그 메세지에 해당하는 내용을 처리한느 방법을
        * 스스로 결정하고, 그 결정한 방법대로 처리할 명령어들을 순차적으로 기술한 것을 메소드라고 한다
        ...
        *
        * 시스템 요구사항
        * 1. 카레이서는 시동걸이, 엑셀레이터 밟기, 브레이크 밟기, 시동 끄기를 할 수 있다
        * 2. 자도아는 시동걸기, 앞으로 가기, 멈추기, 시동끄기
        ...
        * - 메소드 : 시동을 걸어라, 앞으로 가라, 멈춰라, 시동을 꺼라*/
        ...
        while(true){
            ...
            switch (no){
                ...
                default:
                    System.out.println("잘못된 변호를 선택하였습니다."); break;
            }
