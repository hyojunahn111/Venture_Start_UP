...
public class MemberDTO {
    ...
    * 호원번호, 회원명, 나이, 성별, 키 몸무게, 회원탈퇴여부(활성화여부)를 관리할 것이다
    * 값 객체가 가지는 속성(필드)를 추출하는 과정 또한 추상화라고 볼 수 있다
    * DTO클래스를 만들기 위해서는 모든 필드를 private로 만든다*/

    private int number;         //호원번호
    private String name;        //호원명
    private int age;            //나이
    private char gender;        //성별
    private double height;      //키
    private double weight;      //몸무게
    private boolean isActivated;    //호원탈퇴여부(활성화여부)

    /*이렇게 필드만 만들고 나면 private로 접근이 제한되었기 때문에 각 영역에 접근을 할 수 없다
    * public으로 접근을 허용하는 설정자/접근자를 이용해 필드에 직접적으로 접근할 수 있도록 할 것이다*/
