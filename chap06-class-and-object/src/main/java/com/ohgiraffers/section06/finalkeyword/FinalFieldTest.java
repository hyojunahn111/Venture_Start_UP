...
public class FinalFieldTest {
    ...
    /*final
    *   : final은 종단의 의미를 가지느 키워드이다
    *   final 키워드를 사용할 수 있는 위츠는 다양한 편이며 의미가 약간은 다르지만, 결국 변경 불가의 의미이다
    *   1. 지역변수 : 초기화 이후 값 변경 불가
    *   2. 매개변수 : 호출시 전달한 인자 변경 불가
    *   3. 전역변수 : 인스턴스 생성 후 초기화 이후에 값 변경 불가
    *   4. 클래스(static) 변수 : 프로그램 start 이후 값 변경 불가
    *   5. nono-static 메소드 : 메소드 재직성(overriding) 불가
    *   6. static 메소드 : 메소드 재직성(overriding) 불가
    *   7. 클래스 : 상속불가*/
...
//    }
    ...
    /*하지만 초기화 블럭으로는 초기화 할 수 있다*/
    /*지금은 가능한것만 보고 필드에 대해 조금 더 정리한 뒤 초기화 블럭에 대해 다시 살펴보자*/
    static{
        STATIC_DOUBLE = 0.5;
    }
