...
SELECT menu_name, menu_price, orderable_status
  FROM tbl_menu
 WHERE orderable_status = 'Y';				-- Y인 정보만 추출해와라
...
-- WHERE orderable_status != 'Y';
 WHERE orderable_status <> 'Y';			-- 위랑 내용은 같지만 밑에것을 더 많이 사용한다
...
-- < AND와 OR의 우선순위 >
SELECT 1 OR 0 AND 0;
SELECT (1 OR 0) AND 0;			-- AND가 OR보다 우선순위가 빠르다
...
SELECT menu_name, menu_price, category_code
  FROM tbl_menu
 WHERE menu_price BETWEEN 10000.1 AND 20000;
...
-- 메뉴명에 민트를 포함하고,
-- 금액이 10000원 이상이며,
-- category_code가 4인메뉴의
-- 모든 컬럼을 조회하세요
SELECT *
  FROM tbl_menu
 WHERE category_code = 4 AND menu_name LIKE '%민트%' AND menu_price > 10000;
