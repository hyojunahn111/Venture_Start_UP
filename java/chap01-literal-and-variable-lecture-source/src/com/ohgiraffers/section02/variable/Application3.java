...
public class Application3 {
    ...
    /*실무적으로는 굉장이 중요하기 때문에 반드시 숙지해야 하고*/
    /*또한, 규칙에 맞는 올바른 변수명을 짓는 것이 좋은 프로그래밍을 하는 첫 단추이다.*/

    /*변수의 명명규칙
    * 1. 컴파일 에러를 발생시키는 규칙
    * 1-1. 동일한 범위 내에서 동일한 변수명을 가둘 수 없다
    * 1-2 예약어는 사용이 불가능하다. (자바에서 미리 사용하겠다고 compliler와 약속한 키워드들을 예약어라고 한다
    * 1-3 변수명은 대/소문자를 구분한다
    * 1-4 변수명은 숫자로 시작할 수 없다
    * 1-5 특수 기호는 '_', '$' 만 사용 가능하다
    *
    * 2 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 암묵적인규칙
    *
    * 2-1 변수명의 길이 제한은 없다
    ...
    * 2-7 명시형으로 작성할 수 있도록 한다
    * 2-8 boolean형은 의무문으로 가급적 긍정형태로 네이밍한다
    * */
...
//        int 1age = 20;
        ...
        /*2. 컴파팅ㄹ 에러를 발생시키지는 않지만 개발자들끼리의 암묵적인 규칙*/
        /*2-1 변수명의 길이 제한은 없다. 하지만 적당히 작성하는 것이 필요하다*/
        int asdadsaasdadadasdadadadsadasdasdadasdasdadasdasdadadadsadadasdas;
        ...
        /*2-6 전혀엊ㄱ인 변수 이름이 있다면 가급적 사용하도록 한다*/
        int sum = 0;
        int max = 10;
        int main = 0;
        int count = 1;
