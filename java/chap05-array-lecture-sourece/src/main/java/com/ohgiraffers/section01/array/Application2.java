...
public class Application2 {
    public static void main(String[] args) {
        ...
        * 발생한 주소를 레퍼런스 변수(참조형 변수)에 저장하고 이것을 참조하여 사용하기 때문에 참조자료형(reference type)이라고 한다*/
        ...
        /*배열을 할당할 시에는 반드시 배열의 크기를 지정해주어야한다*/
//        iarr = new int[];       //크기를 지정해주지 안하서 에러 발생
        iarr = new int[5];
        carr = new char[10];
        ...
        * stack에 저장된 주소로 heap에 할당된 배열을 찾아갈 수 있다*/
        System.out.println("iaar : " + iarr2);
        System.out.println("carr : " + carr2);

        /*hashcode : 일반적으로 객체의 주소값을 10진수로 변환하여 생성된 객체의 고유한 정수 값을 반환한다*/
        /*동일객체인지 비교할 때 사용할 목적으로 쓰이며, 동등객체를 취급하기 위한 용도로 overring해서 사용한다*/
        ...
        * 참고로 String의 문자열 길이는 메소드로 제공하고 있기 때문에 length()로 사용한다*/
        ...
        /*배열의 사이즈를 변경한 것이 아니고, 새로운 배열을 새엉하여 그 주소값을 레퍼런스 변수에 덮어쓴 것이다*/
        ...
        * 한 번 찾아갈수 있는 주소값을 잃어버린 배열을 다시 참고 불가능하다*/
        darr =null;

        /*NullPointerException 발생함
        * 아무것도 참조하지 않고 null이라는 특수한 값을 참조학 있는 경우 참조연산자를 사용하게 될때 발생하는 에러이다*/
        /**/
//        System.out.println("삭제 후 darr의 길이 : " + darr.length ); //NullPointExceptin발생!;
