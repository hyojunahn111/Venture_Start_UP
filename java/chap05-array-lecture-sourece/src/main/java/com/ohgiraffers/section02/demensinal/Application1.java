...
public class Application1 {
    public static void main(String[] args) {
        ...
        * 4. 생성한 여러개의 1차원 배열에 차례로 접근해서 사용*/
        ...
        /*1. 배열의 주소를 보관할 레퍼런스 변수 언언(stack)*/
        int[][] iarr1;
        int iarr2[][];
        int[] iarrr3[];
        ...
        * 배열의 주소를 관리하는 배열의 길이는 앞 부분에 정수형으로 표기하여 반드시 크기를 지정해 줘야한다*/
...
//        iarr1 = new int[][4];
        ...
        * 앞부분 정수는 주소를 관리하는 배열의 크기, 뒷 부분 정수는 각 인덱스에 할당하는 배열의 길이이다
        * 관리하는 여러 개의 배열의 길이가 동일한 경우 아래와 같이 한 번에 할당할 수도 있다
        * 하지만 여러 개의 배열의 길이가 각각 다른 경우에는 위에서 했던 방식처럼 인덱스별로 배열을 따로 할당해 줘야한다
        * 서로 같은 길이의 여러개의 배열을 하나로 묶어 관리하는 2차원 배열을 가변배열이라고 한다*/

        iarr2 = new int[3][5];

        /*4. 각 배열의 인덱스에 차례로 접근해서 값 출력*/
        /*할당 후에 아무 값을 대입하지 않앋 heap 영역에는 값이 없는 상태로 공간을 생성할 수 없기 때문에 기본값이 들어있다*/
        ...
        /*방금 출력한 내용에석 규칙을 찾아보자
        * 배열의 처음과 끝을 차례로 접근하여 구문 자체를 3번 반복하고 있다
        * 따라서 한 개 인덱스가 가진 배열의 값을 추력하는 반복문을 3번 반복하는 반복문을 작성해보자*/

        /*4-2. 중첩 for문을 이용해서 배열의 값 출력*/
        for(int i = 0; i < iarr1.length; i++){

            /*인덱스의 배열값을 차례로 출력하는 반복문*/
