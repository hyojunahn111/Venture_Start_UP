...
public class User {
    ...
    *           this.필드명 = 매개변스ㅜ;                       //설정자 (setter) 여러 개의 기능을 한 번의 호출로 수행할 수 있다
    * }
    * */
...
//    public User(){}
    ...
    /*2. 매개변수가 있는 생성자*/
    /*초기화할 필드가 여러개인 경우, 초기화하고 싶은 필드의 갯수별로 생성자를 여러개 준비해둘 수 있다*/
    /*id,pwd,name의 초기화를 담당할 생성자*/

    public User(String id, String pwd, String name){

        /*분명 동일한 이름의 생성자는 작성하지 못한다고 했는데 작성했다
        * 매개변수 선언부에 작성한 매개변수의 타입, 객수, 순서에 따라 동일한 생성자 혹은 메소드를
        * 한 클래스 내에 여러개 작성할 수 있게 만든 기술이다 (오버로딩)
        * 오버로딩 부분은 뒤에 별도의 챕터에서 다시 다루고 우선 지금은 매개변수 있는 생성자의 역할에 대해 살펴보자
        * */

        /*매개변수에 있는 생성자의 주 목적은 인스턴스 생성 시점에 매개변수로 전달 받은 값을 이용해서 필드를 초기화한다*/
    ...
    /*3. id, pwd, name, enrollDate의 초기화를 담당할 ㅐㅇ성자(모든 필드를 초기화 하는 생성자)*/
...
//        this.enrollDate = enrollDate;
        ...
        /*3-2 사전에 작성되어 있는 다른 생성자 함수를 이용해서 초기화*/
        /*id, pwd, name필드를 초기화하는 기능을 가진 생성자를 바로 위에서 작성했었다
        * 따라서 중복되는 초기화 내용이 발생하게 되는데, 이 중복되는 초기화 내용을 줄여서 작성할 수 있다*/

        /*this() 사용하기
        * this()는 동일 클래스 내에서 작성한 다른 생성자 메소드를 호출하는 구문이다
        * 골호 안에 매개변수의 타입, 객수, 순서에 맞는 생성자를 호출하고 복귀한다(메소드와 동일함)
        * 리턴되어 돌아오지만 리턴값은 존재하지 않는다
        * this()는 가장 첫 줄에 선언해야 하며, 그렇지 않은 경우 Compile Eroor가 발생한다*/
