...
public class UserDTO {
    ...
    * 그 때 사용할 수 있도ㅗㄱ 규ㅣㄱ을 지정해놓은 java 클래스를 자바빈(java bean)이라고 부른다
    * 지금은 특정 목적에 따라 클래스를 작성하는 규칙이라고 보면 된다*/

    /*자바빈 작성 규칙
    * 1. 자바빈은 특정 패키지에 속해있어야함(default 패키지 사용 금지)
    * 2. 맴버변수의 접근제어자는 private로 선언해야함
    * 3. 기본생성자가 명시적으로 존재하야 한다.( 매개변수 있는 생서자는 선택사항)
    * 4. 모든 맴버변수에 접근 가능한 설정자(setter)와 접근자(getter)가 public으로 작성되어 있어야함
    * 5. 직렬화(Serializable 구현)가 되어야 한다.(선택사항)*/

    /*모든 필드를 private 접근 제한으로 설정*/
    ...
    * (명시하지 않고 추후 매개변수 있는 생성자를 추가할 시 에러 발생에 가능성이 있기 때문이다*/
    ...
    * */
    ...
    public java.util.Date enrollDate(){return enrollDate;}
