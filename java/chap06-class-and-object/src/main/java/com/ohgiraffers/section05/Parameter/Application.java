...
public class Application {
    public static void main(String[] args) {
        ...
        * */
        ...
        /*3. 클래스 자료형*/
        /*사각형의 가로, 세로 길이를 저장할 RectAngle이라는 클래스를 하나 추가한다*/
...
//        pt.testVariableLenthArrayParameter();
        pt.testVariableLenthArrayParameter("홍길동");                          //인자가 아무것도 없는 경우 에러 발생
        pt.testVariableLenthArrayParameter("유관순", "볼링");            //가변인자는 전달하지 않아도 됨
        pt.testVariableLenthArrayParameter("이순신", "볼링", "축구", "당구");
        pt.testVariableLenthArrayParameter("신사임당", new String[]{"테니스", "서예", "떡썰기"});

        /*주의사항. 가변인자는 메소드 오버로딩시 유의해야한다*/
        /*메소드를 오버로딩하고 다시 돌아오면 위에 작성한 호출 구문에 컴파일에러가 발생한다
        * 둘 중 어떤 메소드를 호출하는 것인지에 대해 모호성이 발생했기 때문에 에러가 나는 것이다
