...
public class OverloadingTest {
    ...
    * 동일한 읾의 생성자나 메소드를 여러 개 작성할 수 있게 하기도 했다
    * 그러한 것을 오버로딩(Overloading)이라고 한다
    * */

    /*오버로딩의 사용 이유
    * 매개변수의 종류별로 메소드 내용을 다르게 작성해야 하는 경우들이 종종 있다
    * 이때, 동일한 기능의 메소드를 매개변수에 따라 다르게 이름을 정의하면 복잡하고 관리하기가 매우 어려울 것이다
...
//        private void test(){}             //접근제한자는 메소드 시그니처에 해당하지 않는다(에러남)

        /*반환형에 따른 오버로딩 성립 확인*/
//        public int test() { return 0;}      //반환형은 ㅁ[소드 시그니처에 해당하지 않는다(에러남)
