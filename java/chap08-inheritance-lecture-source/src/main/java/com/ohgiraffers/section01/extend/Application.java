...
public class Application {
    public static void main(String[] args) {
        ...
        *
        * 메소스 재정의(overriding)이란 부모가 가지는 메소드 선언부를 그대로 사용하면서
        * 자식클래스가 정의한 메소드대로 동작하도록 구현 몸체 부분을 새롭게 다시 작성하는 기술이다
        * 메소드 재정의를 하면 메소드 호출할 시 재정의한 메소드가 우선적으로 동작하게 된다
        *
        * 이러한 상속이란 기술을 사용하게 되면 얻게되는 이점은 크게 두 가지로 볼 수 있다
        * 1. 새로운 클래스를 작성할 시 기존에 작성한 클래스를 재사용할 수 있다
        *   1-1 재사용시 생산성을 크게 향상시킬 수 있다(새롭게 작성하는것보다 빠르다)
        *   1-2 공통적으로 사용하는 코드가 부모클래스에 존재하면 수정사항이 생길 시 부모 클래스만 수정해도
        *       전체적으로 적용된다(유지보수성 증가)
        * 2. 클래스간의 계층 관계가 형성되면 다향성의 문법적인 토대가 된다*/
        ...
        *   부모클래스 변경할 시 자식 클래스에 모두 영향을 줄 수 있다
        *   역시 유지보수에 악 영향을 미친다
        ...
        *
        * 객체지향 설계관점에서 바라보는 상속
        * 모든 객체는 자신이 수산한 메세지에 대해 응답해야 하는 챔일을 가지며, 그 책임의 규모는 적절해야 한다
        ...
        * 역할이란 동일한 동작을 수행하는 것을 정의한 것이며, 대체 가능성을 의마한다
        * 부모클래스를 추상화 하는 경우에는 역할의 관점으로 바라봐야 한다
        * 그래야 자식 클래스로 생성한 객체들이 서로 역할을 수행해가며 유연한 코드를 작성할 수 있게 된다
        * 동일한 영향을 가지는 모든 객체는 동일한 메세지를 수신하기는 하지만,
        * 객체별로 그 메세지에 응답하는 방식은 서로 다를 수 있다(다형성)
        *
        *
        * */

        /*Car이라는 클래스를 하나 만들것이다
        * 자동차의 역할을 수행할 수 있는 개체이다. 하지만 기본적인 기능만 수행할 숭 ㅣㅆ다*/
        ...
        /*정상적으로 동작한다
        * 하지만 자동차라고 하기엔 아직 너무 기능들이 초라하다
        * 상속 예시로 활용하기 위해 몇 가지 목적을 가진 자동차들을 조금 더 추가할 예정이다
        * FreCar, RacingCar을 추가할 것이다
        *
        * 소방차, 레이싱카 모두 자동차이다
        * 하지만 모든 자동차는 소방차 혹은 ㅅ쿨버스 혹은 레이싱카가 아니다
        * FireCar is a Car(o) / Car is a FireCar(x)
        * Racing Car is a Car(o) / Car is a RacingCar(x)
        *
        * 이러한 관계가 성립되는 것을 IS-A 관계라고한다
        * IS-A 관계가 성립이 되는 경우에 상속을 사용할 수 있다*/
        ...
        /*이렇게 상속은 부모가 가진 맴버를 사용하면서 확장까지 가능하도록 해두었다
        * 상속은 자식이 부모 맴버에 접근해서 자신것처럼 사용 가능하지만, 반대의 경우는 허용되지 않는다*/
