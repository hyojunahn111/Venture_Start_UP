...
public class EntityManagerGeneratorTests {
    ...
    /*
    * [ 인티티 매니저 팩토리(EntityManagerFactoey)]
    * 엔티티 매니저를 생성할 수 있는 기능을 제공하는 팩토리 클래스이다
    * thread-safe하기 때문에 여러 스레드가 동시에 접근해도 안전하므로 서로 다른 스레드 간 공유해서 재사용한다
    * thread-safe하기한 기능을 요청 스코프마다 생성하기에는 비용(시간, 메모리)부담이 크므로
    * application 스코프와 동일하게 싱글톤으로 생성해서 관리하는 것이 효율적이다
    * 따라서 데이터베이스를 사용하는 애플리케잇녀 당 한 개의 EntityManagerFactory를 생성한다
    * */

    @Test
    @DisplayName("엔티티 매니저 팩토리 생성 확인")
    void testGenerateEneieyManagerFactory(){
        //given
    ...
    void testIsEntityFactorySingletonInstance(){
        //given
